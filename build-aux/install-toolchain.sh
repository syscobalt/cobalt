...
help='This script installs the cobalt toolchain.
...
TARGET: The target of the toolchain. (default: $ARCH-cobalt)
...
[ -z "${PREFIX+x}" ] && PREFIX="$HOME/cobalt-toolchain"
[ -z "$SRCDIR" ] && SRCDIR="$HOME/src"
[ -z "$BUILDDIR" ] && BUILDDIR="$SRCDIR"
[ -z "$ARCH" ] && ARCH=x86_64
[ -z "$TARGET" ] && TARGET=$ARCH-cobalt
...
rm -rf "$SRCDIR/cobalt-binutils" "$SRCDIR/cobalt-gcc"
rm -rf "$BUILDDIR/build-binutils" "$BUILDDIR/build-gcc"

echo Downloading sources...
mkdir -p "$SRCDIR"
cd "$SRCDIR"
git clone $binutils_repo cobalt-binutils
git clone $gcc_repo cobalt-gcc

echo Building binutils...
mkdir -p "$BUILDDIR/build-binutils"
cd "$BUILDDIR/build-binutils"
"$SRCDIR/cobalt-binutils/configure" --target=$TARGET --prefix="$PREFIX" \
...
"$SRCDIR/cobalt-gcc/configure" --target=$TARGET --prefix="$PREFIX" \
